...
def previous_day_group_info(client,__default_channel):
    ...
    from_date = pytz.timezone("US/Eastern").localize(datetime.datetime.now().replace(hour=14,minute=30,second=0, microsecond=0) - datetime.timedelta(days=63))
...
def last_day_joined_users(messages):
    for msg in messages:
        if msg.message == "+1" and msg.reply_to_msg_id is None:
            _id_s.append(getattr(msg,'from_id'))
        elif msg.message =="+1" and (msg.reply_to_msg_id == 8300 or msg.reply_to_msg_id == 8299 or msg.reply_to_msg_id == 5333):
            _id_s.append(getattr(msg,'from_id'))
...
def welcome_to_group(client,group_username):
    ...
    client.send_message('teleipa', msg, parse_mode="Markdown")
