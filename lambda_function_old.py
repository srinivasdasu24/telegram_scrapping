...
def previous_day_group_info(client,__default_channel):
    to_date = pytz.timezone("US/Eastern").localize(datetime.datetime.now().replace(hour=14,minute=30,second=0, microsecond=0)-datetime.timedelta(days=0))
    from_date = pytz.timezone("US/Eastern").localize(datetime.datetime.now().replace(hour=14,minute=30,second=0, microsecond=0) - datetime.timedelta(days=7))
...
def last_day_joined_users(messages):
    global active_users
    for msg in messages:
        if msg.from_id not in active_users:
            active_users[msg.from_id] = 1
        else:
            active_users[msg.from_id] += 1
        #active_users=OrderedDict(sorted(active_users.items(), key=lambda t: t[1]))
        #print(active_users)
...
def welcome_to_group(client,group_username):
    ...
    for k,v in active_users.items():
        if v > 5:
            msg += "["+client.get_entity(k).first_name+(client.get_entity(k).last_name if client.get_entity(k).last_name is not None else '') +"](tg://user?id="+str(k)+")"+", "
    print(msg)
    msg = msg[:-2]
    msg += "\n Active users for last week in group %s" %(group_username)
    client.send_message(client.get_me(), msg, parse_mode="Markdown")
